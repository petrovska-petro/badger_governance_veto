...
def test_execute(deploy, dispute_operation, random_operation):
    ...
    with brownie.reverts("TimelockController: operation is disputed can not be executed"):
...
def test_veto_passed(deploy, dispute_operation):
    ...
    deployer = deploy.deployer 
    contract = deploy.contract
    contract.afterCallDisputed(id , True , "", {"from" : deploy.supremecourt})
    # Operation should be cancelled
    assert(contract.isOperation(id)==False)

# test- if a supreme court decision is false, it should not be disputed after afterCallDisputed
